...
import (
	...
	"github.com/kcloutie/cluster-interceptors/add-changed-files/pkg"
...
func main() {
	...
	s.RegisterInterceptor("add-changed-files", pkg.Interceptor{
...
// 		Logger: logger,
// 	}
// 	serv.RegisterInterceptor("addChangedFiles", pkg.Interceptor{
// 		SecretGetter: interceptors.DefaultSecretGetter(kubeclient.Get(ctx).CoreV1()),
// 	})
...
// 	server.CreateAndValidateCerts(ctx, kubeclient.Get(ctx).CoreV1(), logger, serv, tc.TriggersV1alpha1())

// 	// watch for caCert existence in clusterInterceptor, update with new caCert if its missing in clusterInterceptor
// 	server.UpdateCACertToClusterInterceptorCRD(ctx, serv, tc.TriggersV1alpha1(), logger, time.Minute)
